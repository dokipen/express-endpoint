...
Endpoint.prototype.mount = function(app, methods) {
  var self = this

  if (methods) {
    methods.forEach(function(method) {
      app[method](self.config.path, self.middleware(), self.config.handler);
    });
  } else {
    app.get(this.config.path, this.middleware(), this.config.handler);
  }
  return app;
}
