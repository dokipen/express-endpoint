...
exports = module.exports = function(options) {
  ...
  return function(err, req, res, next) {
    var errors
      , callback = req.endpointParams ? req.endpointParams.callback : undefined;
    ...
    function formatHtml() {
      if (req.endpointConfig.example) {
        req.endpointConfig.exampleUrl = urlparse(
            req.endpointConfig.example, true);
      }
      res.render(config.view,
        { errors: errors,
          config: config,
          // change when path specific error handlers are allowed.
          endpoints: [req.endpointConfig],
          Hash: Hash });
    }
